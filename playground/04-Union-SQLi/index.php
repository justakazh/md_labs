...
<!-- if you want to recode this, please dont remove the coder name :) -->
  ...
  <body>
    <div class="container mt-5 " style="background-color: #fff;">
      ...
      <?php 
      include "../config.php";
      $q = mysqli_query($conn, "SELECT * FROM `article` ");
      while ($data = mysqli_fetch_array($q)){
        ?>
        <div class="row">
          <div class="col-md-12">
            <h3> <a href="read.php?id=<?= $data['id'] ?>"><?= $data['title'] ?></a> </h3>
            <p><?= substr($data['content'], 0, 199) ?>[ <a href="read.php?id=<?= $data['id'] ?>">Read More</a> ]</p>
          </div>
        </div>
        <hr>
